...
class TestHealthz:
    def test_returns_200_with_ok_status(self, client: FlaskClient) -> None:
...
class TestAlertValidation:
    ...
    def test_json_array_returns_400(self, client: FlaskClient) -> None:
        resp = client.post("/alert", json=[])
        assert resp.status_code == 400
        assert "JSON object" in resp.get_json()["error"]

    def test_missing_alerts_key_returns_400(self, client: FlaskClient) -> None:
        resp = client.post("/alert", json={})
        assert resp.status_code == 400
        assert "alerts" in resp.get_json()["error"]

    def test_alerts_not_a_list_returns_400(self, client: FlaskClient) -> None:
...
class TestAlertPayload:
    ...
    def test_empty_alerts_publishes_up(self, client: FlaskClient) -> None:
        resp = self._post(client, [])
        assert resp.status_code == 200
        assert resp.get_json()["published"] == "up"

    def test_internet_down_firing_publishes_down(self, client: FlaskClient) -> None:
        alerts = [{"status": "firing", "labels": {"alertname": "InternetDown"}}]
        resp = self._post(client, alerts)
        assert resp.get_json()["published"] == "down"

    def test_critical_severity_firing_publishes_down(self, client: FlaskClient) -> None:
        alerts = [{"status": "firing", "labels": {"severity": "critical"}}]
        resp = self._post(client, alerts)
        assert resp.get_json()["published"] == "down"

    def test_internet_down_resolved_publishes_up(self, client: FlaskClient) -> None:
        alerts = [{"status": "resolved", "labels": {"alertname": "InternetDown"}}]
        resp = self._post(client, alerts)
        assert resp.get_json()["published"] == "up"

    def test_warning_alert_publishes_up(self, client: FlaskClient) -> None:
    ...
    def test_any_outage_in_mixed_list_publishes_down(self, client: FlaskClient) -> None:
        alerts = [
    ...
    def test_mqtt_failure_returns_502(self, client: FlaskClient) -> None:
        alerts = [{"status": "firing", "labels": {"alertname": "InternetDown"}}]
        with patch(
...
class TestMqttPublish:
    def test_publishes_with_correct_arguments(self, client: FlaskClient) -> None:
        alerts = [{"status": "firing", "labels": {"alertname": "InternetDown"}}]
    ...
    def test_no_auth_when_only_user_set(
        self, client: FlaskClient, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr("mqtt_bridge.service.MQTT_USER", "alice")
