...
class TestMqttAuth:
    def test_returns_auth_dict_when_both_credentials_provided(self):
    ...
    def test_returns_none_when_password_is_none(self):
    ...
    def test_returns_none_when_user_is_empty_string(self):
        assert mqtt_auth(user="", password="secret") is None

    def test_returns_none_when_password_is_empty_string(self):
...
class TestPublish:
    def _publish(self, mock_single, *, payload="up", user=None, password=None):
        publish(
            payload,
            user=user,
            password=password,
    ...
    def test_calls_paho_single_with_correct_arguments(self):
    ...
    def test_passes_auth_when_credentials_provided(self):
        with patch("mqtt_bridge.helper.paho_publish.single") as mock_single:
    ...
    def test_passes_no_auth_when_credentials_absent(self):
        with patch("mqtt_bridge.helper.paho_publish.single") as mock_single:
    ...
    def test_retain_is_always_true(self):
        with patch("mqtt_bridge.helper.paho_publish.single") as mock_single:
            self._publish(mock_single)
